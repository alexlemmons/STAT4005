...
1.
A static graph might be more useful if you were comparing major count based on sex.
...
server <- function(input, output, session) {
  ...
  df_box <- reactive({
    df %>% filter(player == input$playerchoice | player == input$playerchoice2) %>%
      select(player, svperc)
  })
  
  box_plot <- reactive({
    ggplot(df_box, aes(x = player, y = svperc)) +
      geom_boxplot()
  })
